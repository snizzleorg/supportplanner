...
```bash
# Use version from package.json (e.g., if package.json has "version": "0.10.0", builds as v0.10.0)
./build-and-push.sh
...
2. Build and push:
   ...
   ./build-and-push.sh
   ```
   This creates: `your-username/support-planner:v0.10.0` and `:latest`
...
The script automatically prefixes with `v` (e.g., `0.10.0` → `v0.10.0`)
